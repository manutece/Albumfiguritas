...
public class Fabrica {
	...
	List<Figurita> generarSobre(int cantFigus) {
		ArrayList<Figurita> Figuritas = new ArrayList<>();
		for (int i = 0; i < cantFigus * 4; i++) {
			String paisElegido = paisesParticipantes[(int) (Math.random() * 32)];
			int posicionJugador = (int) (Math.random() * 12);
			Figurita figurita = new Figurita(calcularValorBase(paisElegido, posicionJugador),
					(ranking.get(paisElegido) * 100 + posicionJugador), posicionJugador, paisElegido);
	...
	List<Figurita> generarSobreTop10(int cantFigus) {
		...
		for (int i = 0; i < cantFigus * 4; i++) {
			String paisElegido = listadoDeMundialesTop10[(int) (Math.random() * 10)];
			int balonElegido = (Math.random() <= 0.5) ? 1 : 2;
			int posicionJugador = (int) (Math.random() * 20);
			if (balonElegido == 1) {
				Figurita figu = new FiguritaTop10(
						calcularValorBase(balonYPaisPorMundialTop10.get(paisElegido)[0], posicionJugador)+(int)(calcularValorBase(balonYPaisPorMundialTop10.get(paisElegido)[0], posicionJugador) * 0.20) , paisElegido,
						"Oro", posicionJugador, (posicionJugador + 1) * 10000 + balonElegido);
				Figuritas.add(figu);
			}
			if (balonElegido == 2) {
				Figurita figu = new FiguritaTop10(
						calcularValorBase(balonYPaisPorMundialTop10.get(paisElegido)[0], posicionJugador)+(int)(calcularValorBase(balonYPaisPorMundialTop10.get(paisElegido)[0], posicionJugador) * 0.10) , paisElegido,
						"Plata", posicionJugador, (posicionJugador + 1) * 10000 + balonElegido);
				Figuritas.add(figu);
			}
